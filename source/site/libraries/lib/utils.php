...
class XiptLibUtils
{
	...
	function ajaxCheckEmailDuringFacebook(&$args, &$response)
	{
		...
		$email = $args[0];
		if(XiptLibUtils::checkIfEmailAllowed($email))
			return true;
	...
	function ajaxCheckUsernameDuringFacebook(&$args, &$response)
	{
		// as per JomSocial code
		$email = $args[0];
		if(XiptLibUtils::checkIfUsernameAllowed($email))
			return true;
	...
	function ajaxCheckUserName(&$args, &$response)
	{
		// as per JomSocial code
		$uname = $args[0];
		if(XiptLibUtils::checkIfUsernameAllowed($uname))
			return true;
	...
	{
		$config = XiptLibUtils::getParams('', 0);
	...
	/*function checkEditAccessRight($myId, $calleId)
	{
		...
		if(XiptLibUtils::isAdmin($myId, $calleId))
			return true;
	...
	{
		return XiptLibUtils::checkEditAccessRight($myId, $calleId);
	}*/
	...
	{
		XiptLibUtils::XAssert(JFile::exists($imagePath) && JFile::exists($watermarkPath));
