...
// 		}
		...
		XiPTLibraryProfiletypes::getAllowedApps($result,'apps');
		return $result;
	}
	...
	function getPrivacy($userid, $appname){
		$db	 = &$this->getDBO();
		$sql = "SELECT privacy FROM #__community_apps" 
	...
	function getAvailableApps( $enableLimit = true )
	{
		...
		//return application as per user accesibility option
		XiPTLibraryProfiletypes::getAllowedApps($applications,'name');
	...
	function getAppInfo($appname)
	{
		...
		if(empty($instances[$appname]))
		{
			...
			$db	 = &$this->getDBO();
			$sql = "SELECT name FROM #__plugins WHERE `element`='{$appname}'";
			$db->setQuery( $sql );
			$app->title = JText::_($db->loadResult());
